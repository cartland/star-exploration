        ...
        nfcUpdates.addSnapshotListener { snapshot, e ->
            ...
            if (snapshot != null && !snapshot.isEmpty) {
                ...
                snapshot.documents.forEach { document ->
                    ...
                    val resetData = document["resetData"] as? Map<String, String>
                    val url = resetData?.get("url")
                    if (url == RESET_LOGICAL_ID) {
                        Log.d(TAG, "watchForResetTrigger: Logical ID is a reset request")
                        configureWebView()
                    } else {
                        Log.d(TAG, "watchForResetTrigger: Logical ID is not a reset request: $url")
                    }
                }
            } else {
